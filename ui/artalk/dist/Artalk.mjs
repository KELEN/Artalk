...
const blockGfm = __spreadProps(__spreadValues({}, blockNormal), {
  table: gfmTable,
  paragraph: edit(_paragraph).replace("hr", hr).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("table", gfmTable).replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", _tag).getRegex()
});
const blockPedantic = __spreadProps(__spreadValues({}, blockNormal), {
...
const inlinePedantic = __spreadProps(__spreadValues({}, inlineNormal), {
  link: edit(/^!?\[(label)\]\((.*?)\)/).replace("label", _inlineLabel).getRegex(),
  reflink: edit(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label", _inlineLabel).getRegex()
});
const inlineGfm = __spreadProps(__spreadValues({}, inlineNormal), {
...
const inlineBreaks = __spreadProps(__spreadValues({}, inlineGfm), {
...
const markedLinkRenderer = (renderer, orgLinkRenderer) => (href, title, text) => {
  const getLinkOrigin = (link2) => {
    try {
      return new URL(link2).origin;
    } catch (e) {
      return "";
    }
  };
...
const markedImageRenderer = (renderer, orgImageRenderer, { imgLazyLoad }) => (href, title, text) => {
  const html2 = orgImageRenderer.call(renderer, href, title, text);
  if (!imgLazyLoad)
    return html2;
  if (imgLazyLoad === "native" || imgLazyLoad === true)
    return html2.replace(/^<img /, '<img class="lazyload" loading="lazy" ');
  if (imgLazyLoad === "data-src")
    return html2.replace(/^<img /, '<img class="lazyload" ').replace("src=", "data-src=");
  return html2;
};
let instance;
let replacers = [];
...
function initMarked(options) {
  ...
  marked$1.setOptions(__spreadValues(__spreadValues({
...
function simpleMarked(src) {
  return src.replace(
    ...
    /\[(.*?)\]\((.*?)\)/g,
    (_, text, link2) => `<a href="${link2}" target="_blank">${text}</a>`
  ).replace(/\n/g, "<br>");
}
...
class SidebarLayer extends Component {
    ...
    return __async(this, arguments, function* (conf = {}) {
      ...
      if (this.refreshOnShow) {
        ...
        if (this.getDarkMode() !== iFrameSrc.includes("&darkMode=1")) {
          this.iframeLoad(
            $iframe,
            this.getDarkMode() ? iFrameSrc.concat("&darkMode=1") : iFrameSrc.replace("&darkMode=1", "")
          );
        }
      }
      this.authCheck({
  ...
  getDarkMode() {
    return this.conf.darkMode === "auto" ? window.matchMedia("(prefers-color-scheme: dark)").matches : this.conf.darkMode;
  }
...
function applyHeightLimit(obj) {
  ...
  $expandBtn.onclick = (e) => {
    ...
    if (obj.afterExpandBtnClick)
      obj.afterExpandBtnClick(e);
  };
...
function applyScrollableHeightLimit(opt) {
  if (!opt.el)
    return;
  if (opt.el.classList.contains(HEIGHT_LIMIT_SCROLL_CSS))
    return;
...
function renderReplyAt(r) {
  ...
  ((_a = r.$content) == null ? void 0 : _a.firstChild).insertAdjacentElement("afterbegin", r.$replyAt);
}
...
class Upload extends EditorPlug {
  ...
  uploadImg(file) {
    return __async(this, null, function* () {
      ...
      if (!fileExt || !AllowImgExts.includes(String(fileExt[0]).toLowerCase()))
        return;
...
function versionCheck(list2, feVer, beVer) {
  ...
  const errEl = createElement(
    ...
    })} <span class="atk-info">${t("currentVersion")}: ${t("frontend")} ${feVer} / ${t("backend")} ${beVer}</span></div>`
  );
  ...
  list2.$el.parentElement.prepend(errEl);
}
...
function load(ctx) {
  return __async(this, null, function* () {
    var _a;
    const loadedPlugins = /* @__PURE__ */ new Set();
...
//# sourceMappingURL=Artalk.mjs.map
